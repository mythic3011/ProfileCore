...
pub fn history(limit: Option<usize>) {
    ...
    let history_file = if cfg!(windows) {
        ...
        env::var("HOME").ok().map(|h| format!("{}/.bash_history", h))
    };
...
pub fn alias_list() {
    ...
    println!("{}", "=".repeat(60));
    println!("{} ProfileCore provides command aliases through shell integration", "ℹ".cyan());
    println!("Run: {} to see full command list", "profilecore --help".yellow());
    println!("\nCommon aliases:");
