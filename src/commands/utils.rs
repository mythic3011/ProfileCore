...
pub fn random_gen(min: i64, max: i64, count: usize) {
    ...
    println!("{}", "=".repeat(60));
    println!("Range: {} to {}", min.to_string().cyan(), max.to_string().cyan());
    ...
    for i in 0..count {
        ...
        println!("{}: {}", (i + 1).to_string().cyan(), num.to_string().green());
    }
...
pub fn sleep_cmd(seconds: u64) {
    println!("\n{} {} seconds...", "Sleeping for".cyan().bold(), seconds.to_string().yellow());
...
pub fn time_now(utc: bool) {
    ...
    if utc {
        let now = Utc::now();
        println!("UTC:         {}", now.format("%Y-%m-%d %H:%M:%S %Z").to_string().green());
        println!("ISO 8601:    {}", now.to_rfc3339().cyan());
        println!("Unix:        {}", now.timestamp().to_string().yellow());
    } else {
        let now = Local::now();
        println!("Local:       {}", now.format("%Y-%m-%d %H:%M:%S %Z").to_string().green());
        println!("ISO 8601:    {}", now.to_rfc3339().cyan());
        println!("Unix:        {}", now.timestamp().to_string().yellow());
    }
...
pub fn time_zones() {
    ...
    println!("UTC:         {}", utc.format("%H:%M:%S").to_string().green());
    println!("Local:       {} ({})", 
...
pub fn version_info() {
    ...
    let rust_version = option_env!("RUSTC_VERSION").unwrap_or(env!("CARGO_PKG_RUST_VERSION", "unknown"));
    ...
    println!();
    println!("Repository:  {}", "https://github.com/mythic3011/ProfileCore".cyan());
    println!("License:     MIT");
    println!();
}
...
pub fn config_set(key: &str, value: &str) {
    ...
    println!("\n{} Configuration would be saved (feature in development)", "ℹ".cyan());
    println!();
}
